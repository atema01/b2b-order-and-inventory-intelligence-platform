...
const BuyerOrders: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const buyerOrders = allOrders.filter(o => {
      if (o.buyerId !== userId) return false;
      // Buyers can see their own drafts, but not drafts created by others
      if (o.status === OrderStatus.DRAFT && o.createdBy !== 'buyer') return false;
      return true;
    });
  ...
  const handleInvoiceClick = (e: React.MouseEvent, order: Order) => {
    e.stopPropagation();
    const buyer = db.getBuyer(order.buyerId);
  ...
  const canPay = (order: Order) => {
    if (order.status === OrderStatus.PENDING || order.status === OrderStatus.PROCESSING || order.status === OrderStatus.SHIPPED || order.status === OrderStatus.DELIVERED) {
          ...
          filteredOrders.map(order => (
            <div 
              ...
              className={`
                ...
                ${(order.status === OrderStatus.CANCELLED || order.status === OrderStatus.DELETED) ? 'opacity-70 grayscale' : ''}
              `}
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                       <span className={`px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-wide ${getStatusColor(order.status)}`}>
                        {order.status}
                      </span>
                ...
                <div className="flex flex-col items-end gap-2">
                   ...
                   <div className="flex items-center gap-2">
                     {order.status !== OrderStatus.DRAFT && order.status !== OrderStatus.CANCELLED && order.status !== OrderStatus.DELETED && (
                       <button 
