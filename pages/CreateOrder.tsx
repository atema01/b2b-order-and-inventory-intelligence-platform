...
const CreateOrder: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (editId) {
      const order = db.getOrder(editId);
      if (order) {
        setSelectedBuyerId(order.buyerId);
        setOrderDate(order.date);
        setPaymentTerm(order.paymentTerms || 'Immediate');
        setCart(order.items.map(i => ({ productId: i.productId, quantity: i.quantity })));
      }
    }
  }, [editId]);
  ...
  const setQuantity = (productId: string, qty: number) => {
    setCart(prev => {
        ...
        // We allow 0 quantity here to support the input field being cleared (user typing)
        // Cleanup happens on blur
  ...
  const handleFinish = (isDraft: boolean = false) => {
    ...
    if (editId) {
      existingOrder = db.getOrder(editId);
    }
    ...
    const orderPayload: Order = {
      ...
      history: [{ 
        status: isDraft ? 'Draft Created' : 'Order Placed', 
