...
const AddStaff: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      // Automatic ID Generation: S-0001
      const allStaff = db.getAllStaff();
      // Simple logic: parse the numeric part of the last ID or default to 0 if none exist
      let nextIdNum = 1;
      if (allStaff.length > 0) {
         // Attempt to parse the max ID number
      ...
      const newId = `S-${nextIdNum.toString().padStart(4, '0')}`;
            ...
            <div className="grid grid-cols-1 gap-3">
              {(roles.length > 0 ? roles : [{ id: 'R1', name: 'Inventory Specialist', description: 'Warehouse operations access' }]).map(r => (
                <button 
                  key={r.id}
                  type="button"
                  onClick={() => setForm({ ...form, role: r.name })}
                  className={`
                ...
                >
                  <div className="flex justify-between items-center mb-1">
                    <p className={`font-black uppercase text-[11px] tracking-wider ${form.role === r.name ? 'text-white' : 'text-primary'}`}>{r.name}</p>
                    {form.role === r.name && <span className="material-symbols-outlined text-white text-sm">check_circle</span>}
                  </div>
                  <p className={`text-[10px] font-medium leading-relaxed ${form.role === r.name ? 'text-white/70' : 'text-gray-400'}`}>
