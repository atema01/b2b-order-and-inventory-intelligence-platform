...
const OrderDetails: React.FC = () => {
  ...
  const handleConfirmDelete = () => {
    ...
    handleStatusUpdate(OrderStatus.DELETED, reasonText);
        ...
        <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
          ...
          <div className="p-6 space-y-6">
            <div className="flex items-start gap-4">
              ...
              <div>
                <p className="font-black text-slate-800 text-lg leading-tight">{buyer?.companyName}</p>
                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-0.5">Account ID: #{buyer?.id || '---'}</p>
              </div>
            </div>
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                <p className="font-bold text-slate-800 text-sm">{buyer?.phone}</p>
                ...
                <div key={i} className="p-5 flex justify-between items-center">
                  ...
                  <div className="text-right shrink-0">
                    <div className="flex items-center justify-end gap-1.5 mb-1">
                      ...
                      <span className="text-[10px] text-gray-300 font-bold">× {item.priceAtOrder.toLocaleString()}</span>
                    </div>
                    <p className="text-sm font-black text-primary">ETB {(item.quantity * item.priceAtOrder).toLocaleString()}</p>
                  </div>
                </div>
              );
            })}
