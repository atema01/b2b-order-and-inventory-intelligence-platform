...
export const printInvoice = (order: Order, buyer: Buyer | null | undefined, products: Product[]) => {
  ...
  const taxableAmount = order.total - order.tax;
  const taxRate = taxableAmount > 0 ? (order.tax / taxableAmount) * 100 : 0;
    ...
    <body>
      <div class="header">
        ...
        <div style="text-align: right;">
          ...
          <div class="meta">
            <strong>#${order.id.replace('ORD-', '')}</strong><br>
            Date: ${order.date}<br>
            <span class="status-stamp ${order.paymentStatus === 'Paid' ? 'paid' : 'unpaid'}">${order.paymentStatus || 'Unpaid'}</span>
          </div>
        </div>
      </div>
      ...
      <div class="totals">
        ...
        <div class="total-row" style="margin-top: 10px; color: #166534; font-weight: 700;">
          ...
          <span>- ETB ${order.amountPaid.toLocaleString()}</span>
        </div>
        <div class="total-row" style="font-weight: 900; color: #0f172a; font-size: 15px; margin-top: 5px;">
          <span>Balance Due</span>
          <span>ETB ${(order.total - order.amountPaid).toLocaleString()}</span>
        </div>
        ` : ''}
      </div>
