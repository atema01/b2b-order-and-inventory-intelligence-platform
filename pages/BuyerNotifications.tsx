...
const BuyerNotifications: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (userId) {
        setBuyerId(userId);
        // Filter for Buyer notifications
        setNotifications(db.getAllNotifications().filter(n => n.recipientId === userId));
    }
  }, []);

  const handleMarkAllRead = () => {
    if (buyerId) {
        db.markAllNotificationsAsRead(buyerId);
        setNotifications(db.getAllNotifications().filter(n => n.recipientId === buyerId));
    }
  };
