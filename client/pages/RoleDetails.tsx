...
const RoleDetails: React.FC = () => {
    ...
    const fetchRole = async () => {
      ...
      try {
        ...
        const [roleRes, permsRes] = await Promise.all([
          fetch(`/api/roles/${id}`, { credentials: 'include' }),
        ...
        setRole(roleData);
        setEditForm(roleData);
        setAllPermissions((permsData || []).map((p: any) => p.name));
      } catch (err) {
    ...
    return (
      <div className="p-4 lg:p-8 flex items-center justify-center min-h-[50vh]">
        <div className="text-center">
          ...
          <p className="text-slate-600">Loading role...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="p-4 lg:p-8 max-w-5xl mx-auto">
        <div className="bg-red-50 border border-red-200 rounded-xl p-6 text-center">
          <p className="text-red-700 font-medium mb-2">Failed to load role</p>
  ...
  return (
    <div className={`p-4 lg:p-8 max-w-5xl mx-auto space-y-8 ${isEditing ? 'pb-96' : 'pb-40'}`}>
      {/* Header */}
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-3 w-full md:w-auto">
                ...
                <button
                  ...
                  className="flex-1 md:flex-none px-6 py-3 rounded-2xl font-black text-xs uppercase tracking-widest bg-red-50 text-red-600 hover:bg-red-600 hover:text-white transition-all"
                >
                  Delete
                </button>
              </div>
        ...
        <div className="lg:col-span-8 space-y-8">
          ...
          <section className="bg-white rounded-[32px] p-8 border border-gray-100 shadow-sm space-y-8">
            ...
            <div className="grid grid-cols-2 gap-4">
              {(isEditing ? allPermissions : Object.keys(role.permissions)).map((perm) => {
                const enabled = isEditing
                  ? Boolean(editForm.permissions && editForm.permissions[perm])
                  : Boolean(role.permissions[perm]);
                return (
                <button 
                  key={perm}
                  disabled={!isEditing}
