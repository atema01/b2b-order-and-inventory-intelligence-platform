...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, setIsOpen, onLogout }) => {
  ...
  useEffect(() => {
    // Determine permissions based on logged in user context
    if (user && user.type === 'seller') {
      ...
      if (staff) {
        ...
        if (role) {
          ...
          // Fallback for default Admin if role obj missing
          setPermissions({
  ...
  const allNavItems = [
    ...
    { path: '/settings', label: t('nav.settings'), icon: 'settings', permission: 'Any' },
  ];
