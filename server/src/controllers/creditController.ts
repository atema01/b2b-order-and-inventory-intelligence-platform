...
export const updateCreditRequest = async (req: Request, res: Response) => {
  ...
  const allowedStatuses = ['Approved', 'Rejected', 'Partially Approved'];
  ...
  try {
    ...
    let finalApproved = status === 'Rejected' ? 0 : parseNumber(approvedAmount);
    ...
    if (status === 'Approved' || status === 'Partially Approved') {
      await pool.query(
        `UPDATE users
         ...
         WHERE id = $2`,
        [finalApproved, existing.buyer_id]
      );
    }
