...
import { Buyer, Order, BuyerTier } from '../types';
...
const BuyerDetails: React.FC = () => {
  ...
  useEffect(() => {
    if (id) {
      const b = db.getBuyer(id);
      if (b) {
        setBuyer(b);
        setEditForm(b);
        setOrders(db.getAllOrders().filter(o => o.buyerId === id));
      }
    }
  }, [id]);

  if (!buyer) return <div className="p-8">Buyer not found.</div>;

  const handleUpdate = () => {
    if (buyer) {
      const updatedBuyer: Buyer = { ...buyer, ...editForm } as Buyer;
      db.updateBuyer(updatedBuyer);
  ...
  const handlePasswordReset = async (e: React.FormEvent) => {
    e.preventDefault();
    if (buyer && newPassword) {
        const hashedPassword = await hashPassword(newPassword);
        const updatedBuyer = { ...buyer, password: hashedPassword };
        db.updateBuyer(updatedBuyer);
        setBuyer(updatedBuyer);
  ...
  const handleToggleStatus = () => {
    if (buyer) {
      const newStatus: 'Active' | 'Inactive' = buyer.status === 'Active' ? 'Inactive' : 'Active';
      const updatedBuyer: Buyer = { ...buyer, status: newStatus };
      db.updateBuyer(updatedBuyer);
      setBuyer(updatedBuyer);
  ...
  const handleTierChange = (tier: BuyerTier) => {
    let rate = 0;
    if (tier === 'Silver') rate = 0.05;
    if (tier === 'Gold') rate = 0.10;
    if (tier === 'Platinum') rate = 0.15;
    setEditForm(prev => ({ ...prev, tier, discountRate: rate }));
  };
          ...
          {isEditing && (
            <section className="bg-white rounded-[32px] p-8 border border-gray-100 shadow-sm space-y-8 animate-in slide-in-from-bottom-2">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {[
                  { id: 'Bronze', disc: '0%', color: 'border-orange-200 bg-orange-50/30' },
                  { id: 'Silver', disc: '5%', color: 'border-slate-200 bg-slate-50/30' },
                  { id: 'Gold', disc: '10%', color: 'border-amber-200 bg-amber-50/30' },
                  { id: 'Platinum', disc: '15%', color: 'border-indigo-200 bg-indigo-50/30' }
                ].map((t) => (
                  <button 
                    key={t.id}
                    onClick={() => handleTierChange(t.id as BuyerTier)}
                    className={`
                  ...
                  >
                    <div className="flex justify-between items-center mb-1">
                      <p className={`font-black uppercase text-[11px] tracking-wider ${editForm.tier === t.id ? 'text-primary' : 'text-gray-400'}`}>{t.id}</p>
                      {editForm.tier === t.id && <span className="material-symbols-outlined text-primary text-sm">check_circle</span>}
                    </div>
                    <p className={`text-2xl font-black ${editForm.tier === t.id ? 'text-slate-900' : 'text-slate-400'}`}>{t.disc} <span className="text-[10px] font-bold uppercase">Off</span></p>
                  </button>
                ))}
              </div>
            </section>
