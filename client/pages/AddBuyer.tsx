...
const AddBuyer: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
        throw new Error(data.error || 'Failed to register buyer');
      }

      navigate('/buyers');
    } catch (err) {
      console.error('Register buyer error:', err);
      setError(err instanceof Error ? err.message : 'Failed to register buyer');
    } finally {
                  ...
                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                    {tiers.map(tier => (
                      <button 
                        ...
                        `}
                      >
                        <p className={`text-[9px] font-black uppercase tracking-widest ${form.tier === tier.name ? 'text-white/70' : 'text-gray-300'}`}>{tier.name}</p>
                        <p className="text-base font-black">{tier.discountPercentage}% OFF</p>
                      </button>
                    ))}
                  </div>
                )}
