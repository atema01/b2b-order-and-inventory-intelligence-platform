...
const BuyerCredit: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (userId) {
        const b = db.getBuyer(userId);
        if (b) setBuyer(b);
        
        // Get requests for this buyer
        const allReqs = db.getAllCreditRequests();
        setRequests(allReqs.filter(r => r.buyerId === userId).reverse());
    }
  }, []);

  if (!buyer) return <div className="p-8">Loading credit details...</div>;
